...
await clickV('Get started'); await clickV('Agent');
...
await p.locator('.pm-side nav .pm-nav',{hasText:'Analytics'}).first().click(); await p.waitForTimeout(900);
console.log('FULL TEXT:', (await p.locator('main').innerText()).replace(/\s+/g,' ').slice(0,700));
await b.close();
